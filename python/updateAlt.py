...
for line in lines:
    im = line.split()[0]
    ##  for DR6: use _cnt files to determine which pixels of weight files to set to 0
    sky = "mkAlt/"   + im.split('.fits')[0] + '_cnt.fits'      # input sky image
    wgt = "weights/" + im.split('.fits')[0] + '_weight.fits'   # output weight
