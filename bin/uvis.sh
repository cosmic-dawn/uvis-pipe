...
# v3.13: removed P2 and P4, done separately with pssm.sh           (14.aug.23)
##-----------------------------------------------------------------------------
...
#    obFlag=DR5_${FILTER}_obFlag.fits
	obFlag=RefObMask.fits               ###### This for the N-band GTO data only
    if [ ! -e $obFlag ]; then
...
#			ec "########   QUIT HERE after mkMasks with list_special   ########" ; exit
		fi
        if [ $int == "T" ]; then ec "# >>> Interactive mode:" ; askuser; fi
    fi
...
#	echo "#### STOP HERE TO FINISH addSky ####" ; exit 0 
    ...
    if [ -e mkSky.log ] && [ $ns -ge 5 ]; then
        ...
        nexp=400      # exec time: 4.7-5.3 min, doable in 40-45 hrs; ask walltime 48h
		...
		if [ -e list_special ]; then
			ec "# ATTN: removing list_special and continuing with full list"
			dd=$(date "+%y%m%d")
		    cp list_special list_special_mkSky_$dd
#			ec "########   QUIT HERE after mkSky with list_special   ########" ; exit
		fi
        if [ $int == "T" ]; then ec "# >>> Interactive mode:" ; askuser; fi
    fi
...
#			echo "[DEBUG] list is $list with $nn entries"
		...
		if [ -e list_special ]; then
			ec "# ATTN: removing list_special and continuing with full list"
			dd=$(date "+%y%m%d")
		    cp list_special list_special_updateWgts_$dd
#			ec "########   QUIT HERE after updateWeights with list_special   ########" ; exit 0
		fi
        if [ $int == "T" ]; then ec "# >>> Interactive mode:" ; askuser; fi
    fi
